...
class Closure(object):
    def __init__(this, func: collections.abc.Callable):
        this.func = func
    def __call__(this, *args):
        return this.func(*args)

# Utilities
def apply_cc(cc: collections.abc.Callable[[typing.Any], typing.Any], v):
    return cc(v)
...
def evalLambda(arg_names: typing.List[str], body, free: typing.List[str], e: Env) -> LazyBox:
    args_num = len(arg_names)
    # Common cases are optimized.
    # After the CPS pass:
    # There is no function with zero argument.
    if args_num == 0:
       def func():
           ne = extendEnv(e, free)
           return LazyBox(lambda: evalExpr(body, ne))
       return Closure(func)
    # Functions with only one argument are generated by the compiler or defined by the user.
    if args_num == 1:
       def func(arg):
           ne = extendEnv(e, free)
           bindEnv(ne, arg_names[0], arg)
           return LazyBox(lambda: evalExpr(body, ne))
       return Closure(func)
    # These functions are all user-defined.
    if args_num == 2:
       def func(arg1, arg2):
           ne = extendEnv(e, free)
           bindEnv(ne, arg_names[0], arg1)
           bindEnv(ne, arg_names[1], arg2)
           return LazyBox(lambda: evalExpr(body, ne))
       return Closure(func)
    if args_num == 3:
       def func(arg1, arg2, arg3):
           ...
           bindEnv(ne, arg_names[0], arg1)
           bindEnv(ne, arg_names[1], arg2)
           bindEnv(ne, arg_names[2], arg3)
           return LazyBox(lambda: evalExpr(body, ne))
       return Closure(func)
    def func(*args):
        if args_num != len(args):
            return LazyBox(lambda:SchemeException(f"evalLambda <func>: arity mismatch(expected: {args_num} argument(s); given: {args})"))
        ne = extendEnv(e, free)
