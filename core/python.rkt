  ...
  (hasheq
   ...
   (append
    (list
     ...
     "    head = form[0]"
     "    if isinstance(head, Symbol) and head in (Symbol(\"define\"), Symbol(\"set!\")):"
     "        # a value position takes values: this is the same refusal the LB"
     "        # compiler makes for a statement written where an expression belongs"
     "        raise SyntaxError(\"eval: a statement cannot be used as an expression: %s\" % head)"
