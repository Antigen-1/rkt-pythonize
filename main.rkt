...
(module+ test
  ...
  (test '(letrec ((builtin (dynamic-require "builtins" none))
                  (print (#%vm-procedure (=> builtin "print") 1))
                  (tuple (make-procedure (lambda (x) x))))
           (print (eval (App (Prim "+") (tuple (Lambda (tuple "x") (Var "x")) (Datum 1) (Datum 2))) #hasheq{})))
        "3\n")
  (test '(letrec ((builtin (dynamic-require "builtins" none))
                  (print (#%vm-procedure (=> builtin "print") 1)))
           (print (eval (If (Datum #t) (Datum 1) (Datum 2)) #hasheq{})))
        "1\n")
  (test '(letrec ((builtin (dynamic-require "builtins" none))
                  (print (#%vm-procedure (=> builtin "print") 1))
                  (tuple (make-procedure (lambda (x) x))))
             (print (eval (App (Lambda (tuple "x") (Begin (tuple (Set! "x" (Datum 2)) (Var "x")))) (tuple (Datum 1))) #hasheq{})))
        "2\n")
  (test '(letrec ((builtin (dynamic-require "builtins" none))
                  (print (#%vm-procedure (=> builtin "print") 1))
                  (tuple (make-procedure (lambda (x) x)))
                  (sym (gensym "x")))
             (print (eval (App (Lambda (tuple sym) (Var sym)) (tuple (Datum 1))) #hasheq{})))
        "1\n")
  (test '(letrec ((builtin (dynamic-require "builtins" none))
                  (print (#%vm-procedure (=> builtin "print") 1))
                  (tuple (make-procedure (lambda (x) x)))
                  (sym (gensym "x")))
             (print (expr-type (App (Lambda (tuple sym) (Var sym)) (tuple (Datum 1))))))
        "app\n")
)   
