...
;; Tests for rkt-pythonize.  They used to live in the `test` submodule of
;; main.rkt; keeping them in their own module leaves the library free of
;; test-only code.
;;
;; Run them with
;;   $ raco test tests/main.rkt
;; or, for every test in the package,
;;   $ raco test -x -p rkt-pythonize
;;
;; `example-table` is provided for scribblings/rkt-pythonize.scrbl, which
;; renders the tests marked with #:example? #t as documented examples.

(module+ test
  (require rackunit racket/system racket/pretty
           "../main.rkt" "../core/main.rkt")
  ...
  (define (test code output #:example? (example? #f) #:opt? (opt? #t))
    (test-begin
      ...
      (define json (time (compile-scheme-code code #:opt? opt?)))
