...
SRC
  ...
  (test-case "a rest parameter collects the remaining forms"
    (check-python-output
     #<<SRC
...
SRC
  ...
  (test-case "the pass lowers macros into eval of the call form"
    ...
    (check-equal?
     ...
     '(begin (define _macro_signatures (quote #hash((twice . #(1 #f)))))
    ...
    (check-equal?
     ...
     '(begin (define _macro_signatures (quote #hash((m . #(2 #t)))))
             (define (m a b rest) b)))
